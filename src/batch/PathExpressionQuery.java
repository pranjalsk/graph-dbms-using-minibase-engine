...
//		PathExpression pathExp = new PathExpression();
		...
		types[3] = new AttrType(AttrType.attrString);
		types[4] = new AttrType(AttrType.attrString);
		types[5] = new AttrType(AttrType.attrInteger);
		types[6] = new AttrType(AttrType.attrString);
		types[7] = new AttrType(AttrType.attrString);*/
		/*******************************************/
	...
	private Iterator getTriNodeEdgePair(Object[] objExpressions, AttrType[] attrTypes, String ehfName, int numBuf) {
...
/*		outputProjList[8] = new FldSpec(rel2, 1);
		...
		if(attrTypes[1].attrType == AttrType.attrString){
			...
			expr[2].operand2.symbol = new FldSpec(new RelSpec(
					RelSpec.innerRel), 5);
		...
		try {
			efscan1 = new EFileScan(ehfName, attrType, s1_sizes,
					(short) 8, 8, inputProjList, null);
			efscan2 = new EFileScan(ehfName, attrType, s1_sizes,
					(short) 8, 8, inputProjList, null);
			sm = new SortMerge(
					attrType, 8, s1_sizes, attrType, 8, s1_sizes, 8,
					32, 7, 32, numBuf, efscan1, efscan2, false,
...
/*		outputProjList[8] = new FldSpec(rel2, 1);
		...
		try {
			...
			efscan2 = new EFileScan(ehfName, attrType, s2_sizes,
					(short) 8, 8, inputProjList, null);
			sm = new SortMerge(
					jtype, 8, s1_sizes, attrType, 8, s2_sizes, 8,
					32, 7, 32, numBuf, am1, efscan2, false,
					false, order, expr, outputProjList, outputProjList.length);
		}catch (Exception e) {
			e.printStackTrace();
		}
