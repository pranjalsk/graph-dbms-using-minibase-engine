...
public class PathExpressionOperations {
	...
	public static void parsePathExpression(String commandLineInvocation, String graphDBName,String pathExpressionOperationType) throws Exception {
		 ...
		 */
			...
			if(inputArg1.length == 1)
			{
				...
				sourceNID=PathExpressionUtil.lookupAndIterateOverPath(nodeLabel, gdb.nhf,gdb.btf_edge_label);
			    args[0]=sourceNID;
			}
			else
			{
				...
				System.out.println("insideArguments.length"+insideArguments.length);
					int value1=Integer.parseInt(insideArguments[1]);
					int value2=Integer.parseInt(insideArguments[2]);
					int value3=Integer.parseInt(insideArguments[3]);
					int value4=Integer.parseInt(insideArguments[4]);
					int value5=Integer.parseInt(insideArguments[5]);
					Descriptor desc= new Descriptor();
					sourceDesc.set(value1, value2, value3, value4, value5);

				//	sourceNID=PathExpressionUtil.lookupAndIterateOverPath(sourceDesc, gdb.nhf,gdb.btf_edge_label);
			...
			for(int i=1; i<length; i++)
			{
				...
				//Iterator it=PathExpression.pathExpress1(args, attr, gdb.nhf.get_fileName(), gdb.ehf.get_fileName(), gdb.btf_edge_label.get_fileName(), nodeLabel, numbuf, (short)32);	
				}
