...
public class TupleUtils {
	...
	public static int CompareTupleWithTuple(AttrType fldType, Tuple t1, int t1_fld_no, Tuple t2, int t2_fld_no,
		...
		case AttrType.attrDesc: // Compare two descriptors with the given
								// descriptor
			try {
				t1_dsc = t1.getDescFld(t1_fld_no);
				t2_dsc = t2.getDescFld(t2_fld_no);
			} catch (FieldNumberOutOfBoundException e) {
				throw new TupleUtilsException(e, "FieldNumberOutOfBoundException is caught by TupleUtils.java");
			}
			int[] arrayOfNegativeOnes = new int[Descriptor.DESCRIPTOR_SIZE];
			Arrays.fill(arrayOfNegativeOnes, -1);

			int[] targetArray = new int[Descriptor.DESCRIPTOR_SIZE];
			for (int i = 0; i < Descriptor.DESCRIPTOR_SIZE; i++) {
				targetArray[i] = target.get(i);
			}

			// Check if Target Descriptor is [-1,-1,-1,-1,-1]
			if (Arrays.equals(targetArray, arrayOfNegativeOnes)) {
				double distanceBetweenDescriptors = t1_dsc.distance(t2_dsc);
				if (distanceBetweenDescriptors <= distance)
					return 0;
				else
					return 1;
			} else {
	...
	public static int Equal(Tuple t1, Tuple t2, AttrType types[], int len, double distance, Descriptor target)
		...
		int[] targetArray = new int[Descriptor.DESCRIPTOR_SIZE];
		for (j = 0; j < Descriptor.DESCRIPTOR_SIZE; j++) {
			targetArray[j] = target.get(j);
		}

		for (i = 1; i <= len; i++)
			if (types[i - 1].attrType == AttrType.attrDesc) {
				try {
					t1_dsc = t1.getDescFld(i);
					t2_dsc = t2.getDescFld(i);
				} catch (FieldNumberOutOfBoundException e) {
					throw new TupleUtilsException(e, "FieldNumberOutOfBoundException is caught by TupleUtils.java");
				}
	...
	public static double getDistanceBetweenDescriptors(AttrType fldType, Tuple t1, int t1_fld_no, Tuple t2,
			int t2_fld_no) throws IOException, TupleUtilsException {
		Descriptor t1_dsc, t2_dsc;
		try {
			t1_dsc = t1.getDescFld(t1_fld_no);
			t2_dsc = t2.getDescFld(t2_fld_no);
		...
		double distanceBetweenDescriptors = t1_dsc.distance(t2_dsc);
		return distanceBetweenDescriptors;
	}
