...
//for node insert operation we need to set fldCnt
	...
	/**
	 * Convert this field into Descriptor
	 * 
	 * @param fldNo
	 *            the field number
	 * @return the converted Descriptor if success
	 * 
	 * @exception IOException
	 *                I/O errors
	 * @exception FieldNumberOutOfBoundException
	...
	public Descriptor getDescFld(int fldNo) throws IOException, FieldNumberOutOfBoundException {
		Descriptor desc;
		if ((fldNo > 0) && (fldNo <= fldCnt)) {
			desc = Convert.getDescValue(fldOffset[fldNo - 1], data);
			return desc;
		} else
			throw new FieldNumberOutOfBoundException(null, "TUPLE:TUPLE_FLDNO_OUT_OF_BOUND");
	}
	 ...
	 */
	public Tuple setDescFld(int fldNo, Descriptor val) throws IOException, FieldNumberOutOfBoundException {
		if ((fldNo > 0) && (fldNo <= fldCnt)) {
